...
# hint: Deterministic Finite Automaton (DFA)
...
class Solution:
    def myAtoi(self, s: str) -> int:
        ...
        number = []
        for c in arr: # Refactor to use indexes
            if not c.isdigit():
                break
            if c == '0'and not number:
                continue
            number.append(c)
        ...
        return self.int_clamp(''.join(number), is_negative)
    
    
    def int_clamp(self, val, is_negative):
        n = 10
        new_val = ''
        if len(val) < 10:
            new_val = val
            ...
            if is_negative:
                new_val = min(val, '2147483648')
            else:
                new_val = min(val, '2147483647')
        else:
            if is_negative:
                new_val = '2147483648'
            else:
        ...
        return new_val if not is_negative else '-' + new_val
                

############################################# A better solution using state machine from leetcode solution tab #########################################
class StateMachine:
    def __init__(self):
